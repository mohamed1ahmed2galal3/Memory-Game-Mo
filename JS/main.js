...
function memorize(array){
    for(let i = 0 ; i < array.length ; i++){
        array[i].classList.add("is-flipped");
    }

    setTimeout(function(){
        for(let i = 0 ; i < array.length ; i++){
            array[i].classList.remove("is-flipped");
        }
    }, 5000);
    ...
    let memoInterval = setInterval(() => {
        ...
        document.querySelector(".memorize").innerHTML =
            `Memorize : <span>${memoTimer}S</span>`;

        if (memoTimer === 5) {
            clearInterval(memoInterval);
            document.querySelector(".memorize").remove();
        }
    }, 1000);
}
